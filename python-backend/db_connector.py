...
def get_order_count(start_date=None, end_date=None):
    try:
        ...
        df['delivery_date'] = pd.to_datetime(df['delivery_date'], errors='coerce')  # 텍스트를 진짜 날짜 데이터로 변환
        df = df.dropna(subset=['delivery_date'])  # 변환 실패한 날짜가 포함된 행을 자동으로 삭제
        ...
        # 결과를 딕셔너리 형식으로 반환
        return df.to_dict(orient="records")  # 데이터 변환
...
def get_order_items(start_date=None, end_date=None):
    try:
        ...
        if end_date:
            query += f" AND ot.delivery_date <= '{end_date}'"
        query += " GROUP BY it.item_code, it.item_name ORDER BY it.item_code ASC"
        ...
        logging.error(f"get_order_items 에러: {traceback.format_exc()}")
        raise e  # 예외를 외부에서 처리할 수 있게 다시 던짐
...
def get_sales_items(start_date=None, end_date=None):
    try:
        ...
        logging.error(f"get_sales_items 에러: {traceback.format_exc()}")
        raise e  # 예외를 외부에서 처리할 수 있게 다시 던짐
