...
function confirmDelete() {
  return new Promise((resolve) => {
    ...
    const notification = (
      <Notification type="warning" header="휴가 삭제" closable>
        <p>삭제하면 되돌릴 수 없습니다.</p>
        <p style={{marginBottom: 20}}>삭제하시겠습니까?</p>
        <ButtonToolbar>
          <Button appearance="primary" onClick={() => handleResolve(true)}>확인</Button>
          <Button appearance="default" onClick={() => handleResolve(false)}>취소</Button>
        </ButtonToolbar>
      </Notification>
    );
...
const VacaItemsTable = ({ data, columns, onReloading }) => {
  ...
  const deleteVaca = async (v_code) => {
    ...
    try {
      ...
      // console.log(res.status);  // 예: 200, 404, 500. 뭐라고 찍히는지 확인용 콘솔
              ...
              <React.Fragment key={col.dataKey}>
                ...
                {col.dataKey === "v_name" && (
                  <Column width={250} className="text_center">
                    <HeaderCell style={{ backgroundColor: '#f8f9fa' }}>사용기간</HeaderCell>
                    <Cell>
