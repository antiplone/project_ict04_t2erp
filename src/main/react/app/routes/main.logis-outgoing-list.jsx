    ...
    useEffect(() => { // 통신 시작 하겠다.
        fetch("http://localhost:8081/logissales/logisSalesList", { // 스프링부트에 요청한다.
            ...
            res => {
                ...
                // const list = Array.isArray(res) ? res : res?.salesList || [];
                setSalesList(res);
            }
        ).catch(error => {
                ...
                <Table height={400} data={salesListWithRowNum} className="text_center">
                    ...
                    <Table.Column width={150}>
                        ...
                        <Table.Cell dataKey="item_name" style={{ padding: '6px' }}>
                            {salesList => (
                                <Link to={`/main/logis-sales-item-list/${salesList.order_id}`} className="btn btn-primary area_fit wide_fit">주문상세보기</Link>
                            )}
                        </Table.Cell>
                    </Table.Column>
